...
class AWhereAPI(object):
    ...
    def create_field(self, field_id, field_name, center_lat, center_lon, acres=10, farm_id='FarmTest'):
        """
        Performs a HTTP POST request to create and add a Field to your aWhere App.AWhereAPI
        ...
        # Next, we build the request body. Please refer to the docs above for
        # more info.
        fieldBody = {'id': field_id,
                     'name': field_name,
                     'farmId': farm_id,
                     'centerPoint': {'latitude': center_lat,
        ...
        if response.status_code == 201:
            print(
                'Your field "{0}" was successfully created!'.format(field_name))
        else:
            print('An error occurred. Please review the above resonse and try again.')
            
    def get_observed_weather(self, field_id, start_date, end_date):
        """
        Obtains daily observed weather data for specified field and time period
        """
        auth_headers = {
            "Authorization": "Bearer %s" % self.auth_token,
        }
        response = rq.get(self._weather_url + '/{0}/observations/{1},{2}'.format(field_id, start_date, end_date), headers=auth_headers)
